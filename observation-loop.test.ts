...
it("removing its own head ends the check cleanly after that turn", async () => {
	...
	expect(h.calls[0].action).toBe("noop");
	expect(vi.mocked(h.pi.sendUserMessage).mock.calls.map(([message]) => message)).toEqual([
		expect.stringMatching(/^\[pi-hydra long-check\] automatic notice: .*long-check.*Fixture no longer needs this head\./),
	]);
	expect(h.problems()).toEqual([]);
	expect(h.pi.sendMessage).not.toHaveBeenCalled();
});
