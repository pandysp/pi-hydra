...
| [OPENAI-PROTOCOL-STUDY-SPEC.md](OPENAI-PROTOCOL-STUDY-SPEC.md) | **EXACT MATRIX REGISTERED; NO CALLS YET:** fresh OpenAI-only causal studies for terse ENUM and removing `interrupt`. Nine new cases were committed before candidate wording; four exact arms, two configs, two samples, 144 calls, prompt hashes, stop rules, and the blinded one-Sol diagnostic judge are frozen before spend. |
