...
cache probes only and predates almost all of this; start here instead. Counts
are intentionally omitted because research drafts and generated artifacts make
them drift without changing this map.
...
| [GOLDEN-DATASET-V3-RESULTS.md](GOLDEN-DATASET-V3-RESULTS.md) | **v3 FOLDED, THE CURRENT RULER** (`d176183abab2d211`, 2026-08-05): 75 active (26 blocking), the iteration-2 fold of 27 byte-verified settled outcomes. Byte-verification refuted two unanimously voted anchors; the blinded human queue is 11 items. |
...
| [JUDGE-DESIGN-SELECTED.md](JUDGE-DESIGN-SELECTED.md) → [EXPANDED-2Q-IMPLEMENTATION-SPEC.md](EXPANDED-2Q-IMPLEMENTATION-SPEC.md) | **IMPLEMENTED; FINAL SEMANTIC APPROVAL OPEN:** Andreas's expanded 2Q / dual-catalog design. Catalogs, Pi replay judges, exact reconciliation, growth ledger, four-bucket scorer, and public audit are implemented. Current routes passed live probes; the 45-finding sample was terminally valid with 33/45 exact agreement. Eight of 12 disagreements expose the requested-test vs process-advice rule that Andreas must settle before a full pass. The frozen mixed-content audit still supports omitting `unsupportedExtra`. |
...
| [ITERATION2-JUDGE-WAVE-SPEC.md](ITERATION2-JUDGE-WAVE-SPEC.md) | **RECOVERY EXECUTED 2026-08-05; v3 FOLDED.** Andreas decided both gates: the six-record Opus residue completed under the frozen old protocol (4 ratified, 2 dissents queued), and `golden-dataset-v3-fold.mjs` folded the 27 byte-verified settled outcomes — v3 `d176183abab2d211`, 75 active / 26 blocking, checker 8/8. Byte-verification refuted two unanimously voted anchors (BF-d44, SCHED-r-d35); the blinded human queue is 11 items. Sol-side rescore waits on the OpenAI limit; nothing pools with the v2 mechanics sample. |
...
| [CAPSTONE-FROZEN-INPUTS.json](CAPSTONE-FROZEN-INPUTS.json) | Deterministic SHA-256 inventory of the exact provisional dataset basis shown to the judges, all frozen scheduler/exporter/dispatcher/OpenAI trajectory inputs, and the scoring definitions. Refresh when a hashed definition document changes; the frozen judge basis itself remains exact and final-v2 differences are reconciled as a versioned follow-up. |
...
User prompts across all four archived project sessions:
