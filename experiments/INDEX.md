...
| [JUDGE-DESIGN-SELECTED.md](JUDGE-DESIGN-SELECTED.md) → [EXPANDED-2Q-IMPLEMENTATION-SPEC.md](EXPANDED-2Q-IMPLEMENTATION-SPEC.md) | **IMPLEMENTED; FINAL SEMANTIC APPROVAL OPEN:** Andreas's expanded 2Q / dual-catalog design. Catalogs, Pi replay judges, exact reconciliation, growth ledger, four-bucket scorer, and public audit are implemented. Current routes passed live probes; the 45-finding v2 sample was terminally valid with 33/45 exact agreement (the v3 Opus rerun had one tool-use point terminally invalid). Eight of 12 disagreements expose the requested-test vs process-advice rule that Andreas must settle before a full pass. The frozen mixed-content audit still supports omitting `unsupportedExtra`. |
