...
**Still unverified, named.** Older codex models (gpt-5.4, gpt-5.5) pass the same gate but their cache economics are unvalidated (visible in the footer if they misbehave). The OpenAI API-key path shares the serializer but is out of scope for this project (hydra targets subscription auth); it stays gated for any contributor who wants to measure it. An Anthropic live re-verification of the `getApiKey` addition waits on an Anthropic login. On subscription codex, observations spend the same account quota as the driver. And pi-ai keeps a per-session SSE-fallback registry that one websocket connect failure can trip permanently: hydra's observations would then run SSE for the rest of the session — the driver is unaffected (separate module instance), but on days the backend refuses SSE for GPT-5.6, every observation fails with a per-observation error until restart.
