...
4. **No turn or cost ceiling.** Hydra does not stop a head because it has used a set number of model turns or tool calls. Keep its scope and completion criteria clear; turn counts and cost remain visible in `/hydra-stats`. Completion, head deactivation, session shutdown, and cache-sharing safety checks still stop observations; provider and tool limits still apply.
